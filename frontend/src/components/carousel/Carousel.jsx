    ...
    const fetchWeatherData = async () => {
      try {
        const response = await axios.post(
          "http://localhost:5500/recipe/weather-suggestions",
    ...
    const fetchRecipes = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5500/recipe/get-recipes"
        );
        setRecipes(response.data.recipes.slice(0, 5) || []);
      } catch (error) {
  ...
  return (
    <div className="carousel-container">
      ...
      {selectedRecipe && (
        ...
        >
          <ShowRecipe recipe={selectedRecipe} chartReference={chartRef} onClose={closeModal} />
        </Modal>
      )}
    </div>
  );
