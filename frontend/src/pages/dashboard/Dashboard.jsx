...
const Dashboard = () => {
  ...
  const handleDeleteRecipe = async (recipeId) => {
    try {
      await axios.delete(
        `http://localhost:5500/recipe/delete-recipe/${recipeId}`,
        {
          headers: {
  ...
  const handleDeleteAccount = async () => {
    try {
      await axios.delete(`http://localhost:5500/user/delete/${user.email}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
        data: {
      ...
      const fetchRecipes = async () => {
        try {
          const response = await axios.post(
            "http://localhost:5500/recipe/get-recipe-by-userId",
            {
              userId: user.id,
            },
            {
